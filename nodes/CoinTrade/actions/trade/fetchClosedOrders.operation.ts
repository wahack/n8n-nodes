...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[]> {
	...
	for (let i = 0; i < length; i++) {
		try {
			...
			const responseData = await exchanges[platform].fetchClosedOrders(proxy, credentials as any, symbol, since ? +since : pastMonth , limit || 10, params || {})
